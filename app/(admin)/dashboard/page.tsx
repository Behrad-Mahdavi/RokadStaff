    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-[#151C28] rounded-3xl p-6 sm:p-7 border border-[#EAEAEA] dark:border-gray-850 shadow-[3px_3px_0_#202A5A]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.keys(departments).length === 0 ? (
            <div className="text-center py-10 text-sm text-ink-normal/50 dark:text-gray-400 col-span-2">داده‌ای یافت نشد.</div>
          ) : (
      ...
      <div className="bg-white dark:bg-[#151C28] rounded-3xl p-6 sm:p-7 border border-[#EAEAEA] dark:border-gray-850 shadow-[3px_3px_0_#202A5A]">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          ...
          <Link
            href="/reports"
            className="rokad-btn-outline px-4 py-2 text-xs sm:text-sm rounded-xl font-bold dark:bg-[#1C2536] dark:border-gray-700 dark:text-gray-200"
          >
        ...
        {recentReports.length === 0 ? (
          <div className="text-center py-12 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl">
            ...
            <div className="text-base font-bold text-ink-normal/80 dark:text-gray-300">هنوز گزارشی برای امروز ثبت نشده است.</div>
            <div className="text-xs sm:text-sm text-ink-normal/50 dark:text-gray-400 mt-1">کارمندان می‌توانند با ارسال دستور /report به ربات تلگرام گزارش خود را بفرستند.</div>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {recentReports.map((report: any) => {
              ...
              return (
                <div
                  key={report.id}
                  className="p-5 rounded-2xl border border-gray-200 dark:border-gray-800 hover:border-primary hover:shadow-md transition-all duration-200 flex flex-col justify-between bg-[#FCFDFD] dark:bg-[#1C2536]"
                >
                  <div>
                    <div className="flex items-center justify-between mb-2.5">
                      <span className="font-black text-base text-sec dark:text-white">{report.employeeFullName}</span>
                      <span
                    ...
                    <div className="text-xs sm:text-sm text-ink-normal/60 dark:text-gray-400 mb-3 font-medium">
                      دپارتمان: {report.employeeDepartment || "پسرانه"}
                    </div>
                    <p className="text-xs sm:text-sm text-ink-normal/80 dark:text-gray-300 line-clamp-3 bg-white dark:bg-[#151C28] p-3 rounded-xl border border-gray-100 dark:border-gray-800 font-medium leading-relaxed">
                      {cleanText}
                    </p>
                  </div>

                  <div className="pt-3.5 mt-3 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between text-xs text-ink-normal/60 dark:text-gray-400 font-medium">
                    <div className="flex items-center gap-1.5">
                      <Clock className="w-4 h-4" />
                      <span>ساعت ثبت: {formatTehranTime(report.submittedAt)}</span>
                    </div>
                    {report.editedCount > 0 && (
                      <span className="text-xs bg-gray-100 dark:bg-gray-700 text-ink-normal/80 dark:text-gray-300 px-2 py-0.5 rounded-md font-bold">
