...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-ecosystem-light via-white to-club-light/40 p-6 rounded-3xl border-2 border-primary/20 shadow-[3px_3px_0_#59BBAF]">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-primary mb-1">
            <BarChart3 className="w-4 h-4" />
            <span>ماژول پیشرفته گزارش‌گیری و آنالیتیکس</span>
          </div>
          <h1 className="text-2xl font-black text-sec">تحلیل و گزارش‌گیری جامع عملکرد</h1>
        ...
        <button
          ...
          className="rokad-btn-primary px-5 py-3 text-xs rounded-xl flex items-center gap-2 shadow-[2.5px_2.5px_0_#1F413D]"
        >
      ...
      {/* Filter & Range Bar */}
      <div className="bg-white p-5 rounded-2xl border border-[#EAEAEA] shadow-[2px_2px_0_#202A5A] flex flex-col lg:flex-row items-center justify-between gap-4">
        {/* Preset Range Buttons */}
        <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
          <span className="text-xs font-bold text-ink-normal/60 ml-2">بازه سریع:</span>
          {[
            { label: "امروز", days: 0 },
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl border border-gray-200 text-xs font-bold hover:bg-ecosystem-light hover:border-primary/40 hover:text-ecosystem-darker transition-colors"
            >
              {preset.label}
            </button>
          ))}
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <div className="flex items-center gap-2 text-xs bg-gray-50 px-3 py-2 rounded-xl border border-gray-200">
            ...
            />
            <span className="text-gray-500 mr-2">تا:</span>
            <input
              type="date"
              value={to}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <StatCard
          ...
          subtitle={`${toPersianDigits(kpis.totalOnTime)} به‌موقع | ${toPersianDigits(kpis.totalLate)} با تأخیر`}
          icon={Clock}
          theme="male"
        />
      ...
      <div className="bg-white rounded-2xl p-6 border border-[#EAEAEA] shadow-[3px_3px_0_#202A5A]">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h2 className="text-base font-extrabold text-sec">روند زمانی شاخص‌های عملکردی</h2>
            <p className="text-xs text-ink-normal/50 mt-0.5">
        ...
        {/* Visual Trend Bars */}
        {trendData.length === 0 ? (
          <div className="text-center py-16 text-xs text-ink-normal/50">
            داده‌ای برای ترسیم نمودار در این بازه یافت نشد.
          </div>
        ) : (
          <div className="space-y-3 pt-4">
            <div className="h-48 flex items-end gap-2 sm:gap-4 overflow-x-auto pb-4 pt-8 px-2 border-b border-gray-100">
              {trendData.map((pt, i) => (
                <div key={i} className="flex-1 min-w-[40px] flex flex-col items-center gap-2 group">
                  <div className="text-[10px] font-bold text-ink-normal/70 opacity-0 group-hover:opacity-100 transition-opacity">
                    ٪{toPersianDigits(pt.value)}
                  </div>
                  <div className="w-full max-w-[32px] h-32 bg-gray-100 rounded-t-lg relative flex items-end overflow-hidden">
                    <div
                      ...
                      style={{ height: `${Math.max(5, pt.value)}%` }}
                    />
                  </div>
                  <div className="text-[10px] font-medium text-ink-normal/60 rotate-45 sm:rotate-0 mt-1 whitespace-nowrap">
                    {toPersianDigits(pt.dateJalali.slice(5))}
                  </div>
                </div>
              ))}
            </div>
            <div className="flex items-center justify-between text-[11px] text-ink-normal/50 pt-2 px-2">
              <span>راهنما: با نگه داشتن ماوس روی هر ستون، درصد دقیق روز نمایش داده می‌شود.</span>
              <span className="font-bold">مجموع روزهای محاسبه‌شده: {toPersianDigits(trendData.length)} روز</span>
            </div>
          </div>
        )}
      </div>
      ...
      >
        {historyLoading ? (
          <div className="py-12 text-center text-xs text-gray-400">در حال دریافت تاریخچه کارمند...</div>
        ) : employeeHistory ? (
          <div className="space-y-5">
            {/* Meta & Streak Banner */}
            <div className="p-4 rounded-2xl bg-gradient-to-r from-ecosystem-light via-white to-college-light border border-primary/20 flex items-center justify-between">
              <div>
                <h3 className="text-base font-black text-sec">{employeeHistory.employee.fullName}</h3>
                <div className="text-xs text-ink-normal/60 mt-0.5">
                  {employeeHistory.employee.department || "عمومی"} • {employeeHistory.employee.position || "همکار"}
                </div>
              </div>

              <div className="flex items-center gap-2 bg-white px-3.5 py-2 rounded-xl border border-college-normal/30 shadow-sm">
                <Flame className="w-5 h-5 text-college-normal animate-pulse" />
                <div className="text-right">
                  <div className="text-xs font-black text-college-darker">
                    {toPersianDigits(employeeHistory.summary.streak)} روز متوالی
                  </div>
                  <div className="text-[10px] text-ink-normal/50">ثبت بدون وقفه (Streak)</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="p-3 bg-gray-50 rounded-xl border">
                <div className="text-xs text-ink-normal/60">تعداد گزارش‌ها</div>
                <div className="text-lg font-black text-sec mt-1">
                  {toPersianDigits(employeeHistory.summary.totalSubmitted)}
                </div>
              </div>
              <div className="p-3 bg-gray-50 rounded-xl border">
                <div className="text-xs text-ink-normal/60">نرخ به‌موقع بودن</div>
                <div className="text-lg font-black text-primary mt-1">
                  ٪{toPersianDigits(employeeHistory.summary.onTimeRate)}
                </div>
              </div>
              <div className="p-3 bg-gray-50 rounded-xl border">
                <div className="text-xs text-ink-normal/60">نرخ انجام تسک‌ها</div>
                <div className="text-lg font-black text-college-normal mt-1">
            ...
            {/* Recent Reports List */}
            <div>
              <h4 className="text-xs font-bold text-sec mb-2">گزارش‌های ارسالی اخیر:</h4>
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {employeeHistory.reports.length === 0 ? (
                  <div className="text-center py-6 text-xs text-gray-400">گزارشی در این بازه ثبت نشده است.</div>
                ) : (
                  employeeHistory.reports.map((rep: any) => (
                    <div key={rep.id} className="p-3 bg-white rounded-xl border border-gray-200 text-xs space-y-1.5">
                      ...
                      <div className="text-[11px] text-ink-normal/70">
